    ...
    <div id="member-screen" class="hidden">
      ...
      <div class="member-hero" id="hero-card">
        ...
        <div class="member-phone" id="m-email-row" style="font-size:12px;opacity:.85;margin-top:2px">
          ...
          <a href="javascript:;" onclick="openEmailModal()" style="color:inherit;text-decoration:underline;margin-left:6px;font-size:11px">Sửa email</a>
          <a href="javascript:;" onclick="openChangePwModal()" style="color:inherit;text-decoration:underline;margin-left:10px;font-size:11px">Đổi mật khẩu</a>
        </div>
...
<div id="pw-modal" class="modal-overlay hidden" onclick="if(event.target===this)closeChangePwModal()">
  <div class="modal">
    <button class="modal-close" onclick="closeChangePwModal()">✕</button>
    <div class="modal-title">Đổi mật khẩu</div>
    <div id="pw-modal-alert" class="alert hidden"></div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <input type="password" id="pw-new" class="form-input" placeholder="Tối thiểu 6 ký tự">
    </div>
    <div class="form-group">
      <label class="form-label">Nhập lại mật khẩu mới</label>
      <input type="password" id="pw-confirm" class="form-input" placeholder="••••••••">
    </div>
    <div class="modal-actions">
      <button class="btn btn-ghost" style="flex:1" onclick="closeChangePwModal()">Huỷ</button>
      <button class="btn btn-primary" style="flex:1" id="pw-save-btn" onclick="saveChangePw()">Đổi mật khẩu</button>
    </div>
  </div>
</div>
...
function openChangePwModal() {
  ...
  hideAlert('pw-modal-alert');
  document.getElementById('pw-modal').classList.remove('hidden');
}
function closeChangePwModal() { document.getElementById('pw-modal').classList.add('hidden'); }
async function saveChangePw() {
  if (!currentCustomer) return;
  const current = document.getElementById('pw-current').value;
  const newPw   = document.getElementById('pw-new').value;
  const confirm = document.getElementById('pw-confirm').value;
  if (!current) { showAlert('pw-modal-alert','Nhập mật khẩu hiện tại','warn'); return; }
  if (newPw.length < 6) { showAlert('pw-modal-alert','Mật khẩu mới tối thiểu 6 ký tự','warn'); return; }
  if (newPw !== confirm) { showAlert('pw-modal-alert','Mật khẩu nhập lại không khớp','warn'); return; }
  const btn = document.getElementById('pw-save-btn');
  btn.disabled = true; btn.textContent = 'Đang lưu...';
  try {
    const res = await fetch('api/auth.php?action=change_password', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({phone: currentCustomer.phone, current_password: current, new_password: newPw})
    });
    const json = await res.json();
    if (json.error) { showAlert('pw-modal-alert', json.error, 'error'); return; }
    // Cập nhật mật khẩu đã lưu trong localStorage
    const saved = localStorage.getItem('wp_member');
    if (saved) {
      const obj = JSON.parse(saved);
      obj.password = newPw;
      localStorage.setItem('wp_member', JSON.stringify(obj));
    }
    showAlert('pw-modal-alert', 'Đổi mật khẩu thành công!', 'success');
    setTimeout(closeChangePwModal, 1200);
  } catch(e) {
    showAlert('pw-modal-alert','Lỗi kết nối','error');
  } finally {
